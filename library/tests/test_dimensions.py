def test_240_240(GPIO, spidev):
    ...
    display = ST7789(port=0, cs=0, dc=24, width=240, height=240, rotation=0)
...
def test_240_135(GPIO, spidev):
    from OrangePi import ST7789
    display = ST7789(port=0, cs=0, dc=24, width=240, height=135, rotation=0)
    assert display.width == 240
...
def test_320_240(GPIO, spidev):
    from OrangePi import ST7789
    display = ST7789(port=0, cs=0, dc=24, width=320, height=240, rotation=0)
